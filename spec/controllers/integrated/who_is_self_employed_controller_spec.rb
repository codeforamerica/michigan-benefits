...
RSpec.describe Integrated::WhoIsSelfEmployedController do
  describe "#skip?" do
    context "when single member household" do
      it "returns true" do
        application = create(:common_application, :single_member)

        skip_step = Integrated::WhoIsSelfEmployedController.skip?(application)
        expect(skip_step).to be_truthy
      end
    end

    context "when multi member household" do
      context "when someone in household is self_employed" do
        it "returns false" do
      ...
      context "when no one in household is self_employed" do
        it "returns true" do
          application = create(:common_application,
            :multi_member,
  ...
  describe "edit" do
    context "with a current application" do
      it "assigns existing attributes" do
        ...
        expect(form.members.first.self_employed_yes?).to eq(true)
        expect(form.members.second.self_employed_yes?).to eq(true)
      end
    end
  end
  ...
  describe "#update" do
    context "with valid params" do
      ...
      it "updates each member with self_employed info" do
        ...
        expect(member_1.self_employed_no?).to be_truthy
        expect(member_2.self_employed_yes?).to be_truthy
      end
    end
