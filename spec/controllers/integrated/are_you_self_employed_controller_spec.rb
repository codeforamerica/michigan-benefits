...
RSpec.describe Integrated::AreYouSelfEmployedController do
  describe "#skip?" do
    context "when household has one member" do
      it "returns false" do
        application = create(:common_application, members: build_list(:household_member, 1))

        skip_step = Integrated::AreYouSelfEmployedController.skip?(application)
  ...
  describe "#update" do
    context "with valid params" do
      ...
      it "updates the model" do
        ...
        expect(current_app.primary_member.self_employed_yes?).to be_truthy
      end
    end
  end
end
