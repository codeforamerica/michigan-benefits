...
RSpec.describe Integrated::AreYouDisabledController do
  describe "#skip?" do
    context "when household has one member" do
      it "returns false" do
        application = create(:common_application, members: build_list(:household_member, 1))

        skip_step = Integrated::AreYouDisabledController.skip?(application)
  ...
  describe "#update" do
    context "with valid params" do
      ...
      it "updates the models" do
        ...
        expect(current_app.primary_member.disabled_yes?).to eq(true)
      end
    end
  end
end
