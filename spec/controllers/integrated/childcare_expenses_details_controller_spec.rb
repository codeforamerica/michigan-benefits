  ...
  describe "#skip?" do
    context "with a single member household" do
      it "returns true" do
        ...
        skip_step = Integrated::ChildcareExpensesDetailsController.skip?(application)
        expect(skip_step).to eq(true)
      end
    end

    context "with a multimember household" do
      context "has childcare expesnes" do
        it "returns false" do
      ...
      context "has no childcare expenses" do
        it "returns true" do
          application = create(:common_application,
            members: build_list(:household_member, 2),
  ...
  describe "#update" do
    context "with valid params" do
      ...
      it "updates the models" do
        ...
        expense = current_app.expenses.find_by(expense_type: "childcare")
