...
RSpec.describe PaperworkIncomeProofController do
  describe "assigned step" do
    it "defaults for first member with income" do
      primary_member = build(:member, employment_status: "not_employed")
    ...
    context "single member household" do
      ...
      context "with income" do
        it "shows this page" do
          primary_member = build(:member, employment_status: "employed")
          snap_application = create(
            :snap_application,
    ...
    context "multi member household" do
      context "someone has income" do
        it "renders :edit" do
          snap_application = create(
            :snap_application,
            members: build_list(:member, 2, employment_status: "employed"),
          )
          session[:snap_application_id] = snap_application.id
      ...
      context "nobody has income" do
        it "skips this page" do
          ...
          build_list(:member, 2, benefit_application: snap_application)
          session[:snap_application_id] = snap_application.id
  ...
  describe "#update" do
    ...
    context "two-member household" do
      ...
      it "updates has_proof_of_income for the second member and redirects to next step" do
        ...
        expect(subject.next_path).to include("/steps/document-guide")
        expect(response).to redirect_to(subject.next_path)
      end
    end
  end
